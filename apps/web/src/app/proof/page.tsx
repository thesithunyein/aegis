    ...
    <div className="min-h-screen bg-black">
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        <motion.div initial="hidden" animate="visible" variants={fadeUp} className="text-center mb-10">
          ...
          <h1 className="text-3xl font-bold tracking-tight mb-2 text-white">Verify Agent Decisions</h1>
        ...
        {showResult && (
          <motion.div initial="hidden" animate="visible" variants={fadeUp}>
            ...
            <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4 mb-5 flex items-center gap-3">
              ...
              <div>
                <div className="text-sm font-semibold text-green-300">Decision Verified</div>
                <div className="text-xs text-green-400 mt-0.5">All checks passed. TEE attestation confirmed.</div>
              </div>
            </div>
            ...
            <div className="card shadow-google mb-4">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h2 className="text-lg font-semibold mb-1 text-white">{mockVerification.decision.action}</h2>
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                <div className="bg-white/5 rounded-xl p-3 text-center">
                  ...
                  <div className="text-lg font-bold font-mono text-white">{mockVerification.decision.confidence}%</div>
                </div>
                <div className="bg-white/5 rounded-xl p-3 text-center">
                  <div className="text-xs text-gray-400 mb-1">Risk Score</div>
                  <div className="text-lg font-bold font-mono text-white">{mockVerification.decision.riskScore}</div>
                </div>
                <div className="bg-white/5 rounded-xl p-3 text-center">
                  <div className="text-xs text-gray-400 mb-1">Timestamp</div>
                  <div className="text-sm font-mono text-white">{new Date(mockVerification.timestamp).toLocaleTimeString()}</div>
                </div>
              </div>
              ...
              <div className="bg-orange-500/5 border border-orange-500/20 rounded-xl p-4">
                ...
                <p className="text-sm text-gray-300 leading-relaxed">{mockVerification.decision.reasoning}</p>
              </div>
            </div>
            ...
            <div className="card shadow-google mb-4">
              <h3 className="text-sm font-semibold mb-4 flex items-center gap-2 text-white">
                <ShieldIcon className="w-4 h-4 text-green-400" /> TEE Verification
              </h3>
                ...
                {[
                  ...
                  <div key={item.label} className={`flex items-center justify-between py-2.5 ${i < arr.length - 1 ? "border-b border-gray-100" : ""}`}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {item.ok ? <CheckIcon className="w-3.5 h-3.5 text-green-400" /> : <XCircleIcon className="w-3.5 h-3.5 text-red-400" />}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <h3 className="text-sm font-semibold mb-3 flex items-center gap-2 text-white">
                <DatabaseIcon className="w-4 h-4 text-orange-400" /> 0G Storage
                </h3>
                <div className="flex items-center justify-between py-2 mb-3">
                  <span className="text-xs text-gray-400">Root Hash</span>
                  <span className="text-xs font-mono text-orange-400">{mockVerification.storage.hash}</span>
                </div>
              ...
              <div className="card shadow-google">
              <h3 className="text-sm font-semibold mb-3 flex items-center gap-2 text-white">
                <LinkIcon className="w-4 h-4 text-orange-400" /> 0G Chain
                </h3>
                <div className="flex items-center justify-between py-2 mb-3">
                  <span className="text-xs text-gray-400">TX Hash</span>
                  <span className="text-xs font-mono text-orange-400">{mockVerification.chain.txHash}</span>
                </div>
                <div className="flex items-center justify-between py-2">
                  <span className="text-xs text-gray-400">Block</span>
                  <span className="text-xs font-mono text-white">#{mockVerification.chain.blockNumber.toLocaleString()}</span>
                </div>
              </div>
            </div>
          </motion.div>
